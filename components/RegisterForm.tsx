    ...
    return (
        <div className='max-w-[500px] flex flex-col justify-center bg-secondary-bg my-10 h-[592px] p-4'>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col items-start gap-3">
                ...
                <input onChange={(e) => setPasswordConfirm(e.target.value)} className='border border-tertiary-grey rounded text-base h-[40px] w-full pl-2' placeholder='Jelszó megerősítése' required type="password" />
                <label className="flex items-center gap-2 my-1">
                    <input onChange={(e) => setSubscribe(e.target.checked)} type="checkbox" />
                ...
                <label className="flex items-center gap-2 my-1">
                    <input onChange={(e) => setTermsAndConditions(e.target.checked)} required type="checkbox" />
